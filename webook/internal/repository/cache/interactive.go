...
func (i *InteractiveRedisCache) Get(ctx context.Context, biz string, id int64) (domain.Interactive, error) {
	key := i.key(biz, id)
	// 直接使用 HMGet ，即使缓存中没有对应的 key 也不会error
	//i.client.HMGet(ctx,i.key(biz,id),fieldCollectCnt,fieldLikeCnt,fieldReadCnt)
	// 所以你没办法判定，缓存里面是有这个key，但是对应 cnt 都是 0 ，还是说没有这个 key

	// 拿到 key 对应值里面所有的 field
